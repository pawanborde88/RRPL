...
export class AddEditSourceTarget implements OnInit {
  ...
  private initForm(): void {
    ...
    this.targetForm = this.fb.group({
      ...
      site_visit_target: [0, [Validators.required, Validators.min(0)]],
      lead_target: [0, [Validators.required, Validators.min(0)]],
  ...
  private patchFormValues(): void {
    ...
    this.targetForm.patchValue({
      ...
      site_visit_target: editData.site_visit_target,
      lead_target: editData.lead_target,
  ...
  onSave(): void {
    ...
    if (this.isEditMode) {
      payload.source_target_id = this.data.editData.source_target_id;
      payload.updated_by = this.userId;

      this.commonService.editSourceTarget(payload)
        .pipe(
          finalize(() => this.loading.set(false)),
          catchError((err) => {
            this.showSnackBar('Error updating source target', 'error');
            return of(null);
          })
        )
        .subscribe((res) => {
          if (res) {
            this.showSnackBar('Source target updated successfully');
            this.dialogRef.close(true);
          }
        });
    } else {
      payload.created_by = this.userId;
      this.commonService.addSourceTarget(payload)
        .pipe(
          finalize(() => this.loading.set(false)),
          catchError((err) => {
            this.showSnackBar('Error adding source target', 'error');
            return of(null);
          })
        )
        .subscribe((res) => {
          if (res) {
            this.showSnackBar('Source target added successfully');
            this.dialogRef.close(true);
          }
        });
    }
