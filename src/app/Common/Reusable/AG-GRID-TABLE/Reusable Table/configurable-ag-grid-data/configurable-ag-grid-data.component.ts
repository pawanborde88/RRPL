...
export class ConfigurableAgGridDataComponent<T extends TableRowData = TableRowData> implements OnDestroy {
  ...
  readonly rowSelection = computed<RowSelectionOptions | 'single' | 'multiple' | undefined>(() => {
    ...
    return {
      mode: 'multiRow',
      // Temporary fallback: use AG Grid default checkbox behavior.
      checkboxes: true,
      headerCheckbox: true,
      // Prevent selecting all loaded rows; restrict header checkbox to current page.
  ...
  readonly effectiveSelectionColumnDef = computed<ColDef<T> | undefined>(() => {
    ...
    return {
      ...
      ...userDef,
      // Keep native header checkbox enabled while custom header is disabled.
      headerCheckbox: true,
