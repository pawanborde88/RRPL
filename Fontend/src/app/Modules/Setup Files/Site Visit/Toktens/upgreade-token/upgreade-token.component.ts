...
@Component({
  ...
  templateUrl: './upgreade-token.component.html',
  styleUrl: './upgreade-token.component.scss',
})
export class UpgreadeTokenComponent implements OnInit{
  ...
  onSubmit() {
    const formData = {
      ...
      token_type_id: this.upgradeTokens.get('token_type_id')?.value,
      unit_type: this.upgradeTokens.get('unit_type')?.value,
      floor_id: this.upgradeTokens.get('floor_id')?.value,

      wing_id: this.upgradeTokens.get('wing_id')?.value,
      floor_unit_id: this.upgradeTokens.get('floor_unit_id')?.value,
    };

    const apiUrl = `${this.baseUrl}/upgrade_token`;

    this.http.post(apiUrl, formData).subscribe({
      next: (res: any) => {
        if (res.success) {
          // ✅ Patch response values to the form
          this.upgradeTokens.patchValue({
            token_amount: res.token_amount,
            amount_paid: res.amount_paid,
            balance: res.balance,
          });
          this.isTokenUpgraded = true; // Set flag when upgrade is successful
  ...
  payManually() {
    this.showPaymnually = true;
    this.isPaymentDone = true; // Set flag when payment is initiated
  }
}
