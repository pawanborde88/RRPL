...
import { FetchFunctionsService } from '../../../../../Service/fetch-functions.service';
  ...
  @Component({
    ...
    styleUrl: './add-leave-dialog.component.scss'
  })
 ...
 private fetch: FetchFunctionsService,
  ...
  addCustomerForm = new FormGroup({
    ...
    user_id: new FormControl(this.userIdData ,[Validators.required]),
    leave_type_id: new FormControl('' ,[Validators.required]),
    leave_date: new FormControl(''),
    return_to_office:  new FormControl(''),
    crdr: new FormControl(2),
    amount: new FormControl('1'),
    leave_status_id: new FormControl(3),
  ...
  onSubmit() {
    ...
    if (this.addCustomerForm.valid) {
      ...
      console.log(obj);
      this.http.post(`${this.baseUrl}/add_leave_transaction`, obj).subscribe({
        next: (res: any) => {
          console.log(res);
