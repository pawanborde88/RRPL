...
import { DocumentDialogType } from './unified-document-dialog.interfaces';
...
export class PlaceholderReplacerService {
  ...
  buildReceiptReplacements(
    ...
    // Get first receipt for individual field mappings
    const firstReceipt = receiptData && receiptData.length > 0 ? receiptData[0] : {};

    const paymentRows = receiptData
      .map(receipt => `
        <tr>
          ...
          <td>${this.formatIndianCurrency(receipt.received_amount || receipt.amount || 0)}</td>
        </tr>
      `)
      .join('');

    const totalAmount = receiptData.reduce(
      (sum, receipt) => sum + Number(receipt.received_amount || receipt.amount || 0),
      0
    );

    // Project and common data replacements
    ...
    replacements['#DATE#'] = commonData?.receipt_date || firstReceipt?.receipt_date || 'N/A';
    replacements['#WINGFLATNO#'] = this.formatWingFlatNo(commonData);
    replacements['#ChequeInFavor#'] = commonData?.builder_name || 'N/A';

    // Applicant information - handle multiple applicant names
    ...
    const applicantName = applicantNames.length > 0
      ? applicantNames.join(', ')
      : (commonData?.applicant_name || 'N/A');

    // Individual applicant mappings
    replacements['#Applicant#'] = applicantName;
    replacements['#Applicant1#'] = commonData?.applicant_name1 || commonData?.applicant_name || 'N/A';
    replacements['#Applicant2#'] = commonData?.applicant_name2 || 'N/A';
    replacements['#Applicant3#'] = commonData?.applicant_name3 || 'N/A';
    replacements['#Applicant1MobileNo#'] = commonData?.applicant_mobile || 'N/A';
    replacements['#Applicant1Email#'] = commonData?.applicant_email || 'N/A';

    // AllApplicants - join all applicant names with comma (only non-null applicants)
    replacements['#AllApplicants#'] = applicantNames.length > 0
      ? applicantNames.join(', ')
      : (commonData?.applicant_name || 'N/A');
    // Receipt-specific fields from first receipt (data array)
    replacements['#TRNO#'] = firstReceipt?.trn_no || 'N/A';
    replacements['#RECEIPTNO#'] = firstReceipt?.receipt_no || 'N/A';
    replacements['#PaymentMode#'] = firstReceipt?.payment_mode || 'N/A';
    replacements['#BankName#'] = firstReceipt?.bank_name || 'N/A';
    replacements['#Type#'] = firstReceipt?.receipt_type || 'N/A';
    replacements['#ReceiptPaymentDate#'] = this.formatDate(firstReceipt?.receipt_date) || 'N/A';
    replacements['#Amount#'] = this.formatIndianCurrency(firstReceipt?.received_amount || firstReceipt?.amount || 0);
    replacements['#ChequeNo#'] = firstReceipt?.trn_no || 'N/A';
    replacements['#TransactionID#'] = firstReceipt?.trn_no || 'N/A';
    replacements['#ChequeDate#'] = this.formatDate(firstReceipt?.trn_date || commonData?.trn_date) || 'N/A';

    // Amount fields - prefer from first receipt, fallback to common_data
    // Ensure proper number conversion - received_amount takes priority over amount
    // Handle null/undefined properly (0 is a valid amount)
    let receivedAmount = 0;
    if (firstReceipt?.received_amount != null) {
      receivedAmount = Number(firstReceipt.received_amount);
    } else if (firstReceipt?.amount != null) {
      receivedAmount = Number(firstReceipt.amount);
    } else if (commonData?.received_amount != null) {
      receivedAmount = Number(commonData.received_amount);
    }
    replacements['#ReceivedAmount#'] = this.formatIndianCurrency(receivedAmount);
    // AmountInWords should represent the received amount in words
    // Support both case variations for compatibility
    const amountInWords = this.numberToWords(receivedAmount);
    replacements['#AmountInWords#'] = amountInWords;
    replacements['#AmountinWords#'] = amountInWords;
    replacements['#TotalPayable#'] = this.formatIndianCurrency(totalAmount);

    // Replace payment rows pattern
    const paymentRowPattern = /<tr>\s*<td[^>]*>#ReceiptPaymentDate#<\/td>\s*<td[^>]*>#RECEIPTNO#<\/td>\s*<td[^>]*>Towards #Type#<\/td>\s*<td[^>]*>#TRNO#<\/td>\s*<td[^>]*>#PaymentMode#(?:-?#BankName#)?<\/td>\s*<td[^>]*>#Amount#<\/td>\s*<\/tr>/;

    return { ...replacements, __paymentRows__: paymentRows, __paymentRowPattern__: paymentRowPattern.toString() };
  }
